...
import { User as UserIcon, Beef, Heart, MessageCircle, AlertTriangle, Lightbulb, Award, Megaphone, MoreVertical, Edit2, Trash2 } from 'lucide-react';
...
export default function CommunityElevagePage() {
  ...
  const loadPosts = async () => {
    try {
      ...
      const listingPosts = allListings
        .filter((listing: any) => {
          // Check if listing belongs to elevage domain
          const isElevage = 
            listing.category_id === 'elevage' || 
            listing.domain === 'elevage' ||
      ...
      let allPosts = [...demoPosts.filter((post: any) => post.domain === 'elevage'), ...listingPosts];
  ...
  const getActivityTypeColor = (type: string) => {
    const colors: { [key: string]: string } = {
      ...
      producer: 'bg-gradient-to-br from-amber-500/30 to-amber-600/20 border border-amber-400/30 text-amber-200',
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      >
        <div className="text-amber-500/[0.12]">
      ...
      >
        <motion.div 
          ...
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full blur-[120px] bg-amber-500/[0.06]"
        />
      </motion.div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          ...
          {user && (
            ...
            >
              <div className="w-12 h-12 rounded-full bg-amber-500/30 border-2 border-amber-500/50 flex items-center justify-center text-white shadow-lg hover:shadow-xl transition-all transform hover:scale-110">
              ...
              <div className="absolute top-14 right-0 bg-white/10 backdrop-blur-md rounded-xl shadow-xl px-4 py-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap border border-amber-500/30">
        ...
        >
          <div className="flex items-center gap-4 mb-3">
            <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-amber-500/30 to-amber-600/20 border border-amber-400/30 flex items-center justify-center shadow-xl">
              <Beef className="h-8 w-8 text-amber-300" strokeWidth={2.5} />
            </div>
            <div>
        ...
        >
          <button
            ...
            style={{
              background: theme === 'light'
                ? (filter === 'all' ? '#F59E0B' : '#FFFFFF')
                : (filter === 'all' ? 'rgba(251, 146, 60, 0.3)' : 'rgba(255, 255, 255, 0.1)'),
              borderColor: theme === 'light'
                ? (filter === 'all' ? '#F59E0B' : '#D1D5DB')
                : (filter === 'all' ? 'rgba(251, 146, 60, 0.5)' : 'rgba(255, 255, 255, 0.2)'),
              color: theme === 'light'
          ...
          <button
            ...
            style={{
              background: theme === 'light'
                ? (filter === 'tip' ? '#F59E0B' : '#FFFFFF')
                : (filter === 'tip' ? 'rgba(251, 146, 60, 0.3)' : 'rgba(255, 255, 255, 0.1)'),
              borderColor: theme === 'light'
                ? (filter === 'tip' ? '#F59E0B' : '#D1D5DB')
                : (filter === 'tip' ? 'rgba(251, 146, 60, 0.5)' : 'rgba(255, 255, 255, 0.2)'),
              color: theme === 'light'
          ...
          <button
            ...
            style={{
              background: theme === 'light'
                ? (filter === 'warning' ? '#EF4444' : '#FFFFFF')
                : (filter === 'warning' ? 'rgba(239, 68, 68, 0.3)' : 'rgba(255, 255, 255, 0.1)'),
              borderColor: theme === 'light'
            ...
            {posts.map((post) => (
              ...
              >
                {post.images && post.images.length > 0 && (
                  <div className="h-64 bg-gradient-to-br from-amber-500/20 to-orange-500/20 backdrop-blur-sm relative overflow-hidden">
                    <img
                      src={post.images[0]}
                      alt={post.title}
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`px-3 py-1.5 rounded-xl text-xs font-bold shadow-md ${
                          ...
                          post.type === 'tip' ? 'bg-amber-500/30 border border-amber-400/30 text-amber-200' :
                    ...
                    {commentingPost === post.id && (
                      <div className="mt-3">
                        <textarea
                          ...
                          className="w-full rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 resize-none"
                          style={{
