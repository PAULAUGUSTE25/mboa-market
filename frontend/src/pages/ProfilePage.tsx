...
import { User, Edit3, MapPin, Sprout, ShoppingCart, PackageSearch, Wheat, Beef, LogOut, Trash2, Users, Shield } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="lg:col-span-1">
            <div 
              className="rounded-2xl backdrop-blur-xl shadow-2xl p-6 sm:p-8 border border-white/10 bg-black/40"
            >
              ...
              <div className="flex flex-col items-center text-center mb-6 sm:mb-8">
                ...
                <div className="relative w-24 h-24 sm:w-28 sm:h-28 mx-auto mb-3 sm:mb-4">
                  <div className="w-full h-full rounded-full bg-gradient-to-br from-emerald-500/30 to-teal-600/30 flex items-center justify-center border-2 border-emerald-500/50 shadow-lg backdrop-blur-sm">
                    <User className="h-12 w-12 sm:h-14 sm:w-14 text-emerald-400" strokeWidth={1.5} />
                  </div>
                  {/* Status Badge Responsive */}
                  <div className="absolute bottom-0.5 right-0.5 sm:bottom-1 sm:right-1 w-5 h-5 sm:w-6 sm:h-6 bg-emerald-500 rounded-full border-2 border-black shadow-md"></div>
                </div>
                <h2 className="text-lg sm:text-xl font-bold mb-2 text-white" style={{ fontFamily: 'Inter, Plus Jakarta Sans, sans-serif' }}>
                  {user.profile?.display_name || 'Utilisateur'}
                </h2>
                <p className="text-xs sm:text-sm font-normal mb-1 text-white/60">{user.phone}</p>
                {user.email && (
                  <p className="text-xs sm:text-sm font-normal text-white/60">{user.email}</p>
                )}
              </div>

              {/* Activity Type Selector Responsive */}
              <div className="mb-4 sm:mb-6">
                <label className="block text-xs font-medium mb-3 text-white/60">
                  Type d'activité
                </label>
                {showActivitySelector ? (
                  <div className="space-y-2">
                    {[
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-3 p-2.5 sm:p-3 rounded-xl transition-all border ${
                      ...
                      >
                        <div className="flex-shrink-0">
                          {option.value === 'seed_provider' && <PackageSearch className="h-5 w-5" strokeWidth={2} />}
                          {option.value === 'producer' && <Sprout className="h-5 w-5" strokeWidth={2} />}
                          {option.value === 'buyer' && <ShoppingCart className="h-5 w-5" strokeWidth={2} />}
                        </div>
                  ...
                  <button
                    ...
                    className="w-full p-2.5 sm:p-3 rounded-2xl font-semibold transition-all flex items-center justify-between border bg-emerald-600 text-white border-emerald-500/50 shadow-lg hover:bg-emerald-500 text-sm sm:text-base"
                  >
              ...
              <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <label className="block text-xs font-medium text-white/60 mb-1">Région</label>
                  {editMode ? (
                    <input
                      type="text"
                      value={formData.region}
                      onChange={(e) => setFormData({ ...formData, region: e.target.value })}
                      className="w-full px-3 py-2 border border-white/20 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white/10 text-white placeholder-white/40 transition-all"
                    />
                  ) : (
                    <p className="text-sm sm:text-base font-semibold text-white">{user.profile?.region || 'Non spécifié'}</p>
                  )}
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <MapPin className="h-4 w-4 text-emerald-400 mt-1" />
                  <div className="flex-1">
                    <label className="block text-xs font-medium text-white/60 mb-1">Localité</label>
                  {editMode ? (
                    <input
                      type="text"
                      value={formData.locality}
                      onChange={(e) => setFormData({ ...formData, locality: e.target.value })}
                      className="w-full px-3 py-2 border border-white/20 rounded-xl text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white/10 text-white placeholder-white/40 transition-all"
                    />
                  ) : (
                    <p className="text-sm sm:text-base font-semibold text-white">{user.profile?.locality || 'Non spécifié'}</p>
                  )}
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  {formData.domain === 'agriculture' ? (
                    <Wheat className="h-4 w-4 text-emerald-400 mt-1" />
                  ) : (
                    <Beef className="h-4 w-4 text-amber-400 mt-1" />
                  )}
                  <div className="flex-1">
                    <label className="block text-xs font-medium text-white/60 mb-2">Domaine d'activité</label>
                    {editMode ? (
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          ...
                          className={`p-3 rounded-xl font-semibold transition-all duration-200 flex items-center justify-center gap-2 border ${
                            formData.domain === 'agriculture'
                        ...
                        >
                          ...
                          <span className="text-sm">Agri</span>
                        </button>
                        <button
                          type="button"
                          onClick={() => {
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`px-3 py-1.5 border text-sm font-bold rounded-full flex items-center gap-2 ${
                          formData.domain === 'agriculture'
                ...
                <div className="flex items-start gap-3">
                  {formData.domain === 'agriculture' ? (
                    <Sprout className="h-4 w-4 text-emerald-400 mt-1" />
                  ) : (
                    <Beef className="h-4 w-4 text-amber-400 mt-1" />
                  )}
                  <div className="flex-1">
                    <label className="block text-xs font-medium text-white/60 mb-1">
                      ...
                      {editMode ? (
                        <select
                          ...
                          className="w-full px-4 py-3 border border-white/20 rounded-xl text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-black/60 text-white transition-all duration-200"
                        >
                          {formData.domain === 'elevage' ? (
                        ...
                        <p className="text-sm sm:text-base font-semibold text-white">{formData.crop || 'Non spécifié'}</p>
                      )}
                    </div>
                  </div>
              </div>
              ...
              <div className="space-y-2.5 sm:space-y-3">
                {editMode ? (
                  <>
                    <button
                      ...
                      className="w-full border border-emerald-500/50 bg-emerald-600 hover:bg-emerald-500 text-white py-2.5 sm:py-3 rounded-2xl font-semibold transition-all shadow-md hover:shadow-lg text-sm sm:text-base"
                    >
                      Enregistrer
                    </button>
                    <button
                      ...
                      className="w-full border border-white/20 bg-white/5 hover:bg-white/10 text-white py-2.5 sm:py-3 rounded-2xl font-semibold transition-all text-sm sm:text-base"
                    >
                      Annuler
                    </button>
                  </>
                  ...
                  <button
                    ...
                    className="w-full border border-emerald-500/50 bg-emerald-600 hover:bg-emerald-500 text-white py-2.5 sm:py-3 rounded-2xl font-semibold transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 text-sm sm:text-base"
                  >
                    <Edit3 className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                    Modifier le profil
                  </button>
                ...
                <button
                  ...
                  className="w-full py-2.5 sm:py-3 rounded-[16px] font-medium transition-all flex items-center justify-center gap-2 text-sm sm:text-base text-white/70 hover:text-white hover:bg-white/10 border border-transparent hover:border-white/20"
                >
                ...
                <button
                  ...
                  className="w-full py-2.5 sm:py-3 rounded-[16px] font-medium transition-all flex items-center justify-center gap-2 text-sm sm:text-base text-red-400 hover:text-red-300 hover:bg-red-500/10 border border-transparent hover:border-red-500/30"
                >
          ...
          <div className="lg:col-span-2">
            <div 
              className="rounded-2xl backdrop-blur-xl shadow-2xl p-6 sm:p-8 border border-white/10 bg-black/40"
            >
              <h3 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-white" style={{ fontFamily: 'Inter, Plus Jakarta Sans, sans-serif' }}>
              ...
              {loading ? (
                <div className="text-center py-8 sm:py-12">
                  ...
                  <p className="mt-3 sm:mt-4 text-sm sm:text-base text-white/60">Chargement...</p>
                </div>
              ) : myListings.length === 0 ? (
                <div className="text-center py-8 sm:py-12">
                  <p className="text-base sm:text-lg mb-3 sm:mb-4 text-white/60">Aucune annonce pour le moment</p>
                  <button
                    onClick={() => navigate('/listings/create')}
                    className="border border-emerald-500/50 bg-emerald-600 hover:bg-emerald-500 text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl font-semibold transition-all shadow-lg text-sm sm:text-base"
                  >
                    ➞ Créer une annonce
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  {myListings.map((listing) => (
                    <div
                      key={listing.id}
                      className="rounded-2xl p-4 sm:p-5 border transition-all duration-300 relative group/card backdrop-blur-sm bg-black/40 border-white/10 hover:border-emerald-500/50 hover:bg-black/60 shadow-lg"
                    >
                      {/* Vignette Image Responsive */}
                      <div className="flex gap-3 sm:gap-4 mb-3 sm:mb-4">
                        {listing.images && listing.images.length > 0 ? (
                          <div className="w-20 h-20 sm:w-24 sm:h-24 bg-white/5 rounded-[12px] sm:rounded-[16px] overflow-hidden flex-shrink-0 border border-white/10">
                            <img
                          ...
                          <div className="w-20 h-20 sm:w-24 sm:h-24 bg-white/5 rounded-[12px] sm:rounded-[16px] flex items-center justify-center flex-shrink-0 border border-white/10">
                            <Sprout className="h-8 w-8 sm:h-10 sm:w-10 text-white/40" />
                          </div>
                        )}
                        {/* Titres Responsive */}
                        <div className="flex-1 min-w-0">
                          <h4 className="font-bold text-sm sm:text-base mb-1 truncate text-white">{listing.title}</h4>
                          <p className="text-xs sm:text-sm line-clamp-2 text-white/60">{listing.description}</p>
                        </div>
                      </div>
                        ...
                        <div className="flex items-center gap-2 sm:gap-3">
                          ...
                          <div className="flex items-center gap-1.5 sm:gap-2">
                            <div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${
                              (listing.status === 'active' || listing.status === 'PUBLISHED') ? 'bg-emerald-500' : 'bg-gray-400'
                            }`}></div>
            ...
            <div className="mt-6 sm:mt-8 grid grid-cols-2 gap-3 sm:gap-4 md:gap-6">
              <button
                ...
                style={{
                  ...
                  borderColor: theme === 'light' ? '#2E7D32' : 'rgba(16, 185, 129, 0.2)',
              ...
              <button
                ...
                style={{
                  background: theme === 'light' ? '#FFFFFF' : 'rgba(255, 255, 255, 0.05)',
                  borderColor: theme === 'light' ? '#B71C1C' : 'rgba(183, 28, 28, 0.2)',
                  boxShadow: theme === 'light' ? '0 4px 12px rgba(0, 0, 0, 0.1)' : 'none'
                }}
              >
                {/* Icône SVG Responsive */}
                <Users className="h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 text-red-400 mx-auto mb-2 sm:mb-3 group-hover:scale-110 transition-transform" strokeWidth={1.5} />
